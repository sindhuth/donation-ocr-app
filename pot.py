...
if user_role == 'admin':
    ...
    if st.session_state.get('event_stopped', False):
        ...
        with col_right:
            ...
            # Calculate number of visible coins (max 25 when overflowing)
            num_coins_visible = min(int((progress_percentage / 100) * 25), 25)
            fill_height = min(int(progress_percentage * 1.85), 185)  # Max at pot height
            
            # Generate coin positions for inside the pot
            coins_html = ""
            for i in range(num_coins_visible):
                # Random but deterministic positions based on index
                left_pos = (i * 37) % 160 + 10
                bottom_pos = (i * 8) % fill_height if fill_height > 0 else 0
                size = 22 + (i % 3) * 4  # Vary sizes 22-30px
                rotation = (i * 45) % 360
                
                coins_html += f'''
                <div class="coin-piece" style="
                    position: absolute;
                    left: {left_pos}px;
                ...
                for idx, (left, top, size) in enumerate(overflow_positions):
                    ...
                    <div class="overflow-coin" style="
                        ...
                        box-shadow: 
                            ...
                            inset 1px 1px 2px rgba(255,255,255,0.5);
                        border: 1px solid #DAA520;
                        animation: coinPop 0.6s ease {delay}s backwards;
                    "></div>
                    '''
            ...
            components.html(pot_html, height=350)
